    ...
    <section className="mx-auto max-w-7xl px-6 py-20">
      <div className="text-center max-w-3xl mx-auto">
        <span className="inline-flex items-center gap-2 text-xs px-3 py-1 rounded-full border border-primary/30 bg-primary/10 text-primary">
          {isGerman ? "Sender & Live-Events" : "Channels & Live Events"}
        </span>
        <h2 className="mt-4 text-3xl sm:text-4xl font-bold">
          {isGerman ? "Entdecke unsere " : "Explore our "}<span className="text-gradient-green">{isGerman ? "riesige Senderbibliothek" : "massive channel library"}</span>
        </h2>
        <p className="mt-4 text-muted-foreground">
          {isGerman ? "Live-Sport, große Events und Blockbuster-Unterhaltung — Champions League, Formel 1, Premier League und tausende Sender weltweit an einem Ort." : "Live sports, major events & blockbuster entertainment — Champions League, Formula 1, Premier League and thousands more channels from across the world, all in one place."}
        </p>
      </div>
      ...
      <p className="mt-8 text-center text-xs text-muted-foreground">
        🔞 {isGerman ? "Adult-Inhalte (18+) sind nicht standardmäßig enthalten — nur auf Anfrage verfügbar." : "Adult (18+) content is not included by default — available upon request only."}
      </p>

      <div className="mt-8 flex items-center justify-between rounded-xl border border-border bg-surface/40 p-4">
        <span className="text-sm text-muted-foreground">
          {isGerman ? "Neue Filme und Serien werden regelmäßig hinzugefügt." : "Latest movies and series added regularly."}
        </span>
        <Link to="/pricing" className="text-sm font-medium px-4 py-2 rounded-md bg-gradient-primary text-primary-foreground">
          {isGerman ? "Vollen Zugriff erhalten" : "Get full access"} →
        </Link>
      </div>
    </section>
  );
